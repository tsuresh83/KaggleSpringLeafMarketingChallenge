...
my.glm = h2o.glm(x=independent, y=dependent, training_frame=part1h20, validation_frame = part2h20,family="binomial", standardize=F,
                 lambda_search=TRUE)
h2o.saveModel(my.glm,path = outputFolder)

my.dl = h2o.deeplearning(x=independent, y=dependent, training_frame = part2h20,balance_classes =T,validation_frame = part3h20)
h2o.saveModel(my.dl,path = outputFolder,epochs=100)
my.rf = h2o.randomForest(x=independent, y=dependent, training_frame=part3h20,balance_classes =T,validation_frame = part4h20,ntrees = 1000, max_depth=6,binomial_double_trees=T)
h2o.saveModel(my.rf,path = outputFolder)
my.gbm = h2o.gbm(x=independent, y=dependent, training_frame=part4h20,balance_classes =T,validation_frame = part1h20,ntrees = 2000, max_depth=6,learn_rate=0.01)
...
p0s$Mean <- rowMeans(p0s[,c(2,3)])
submission$target <- p0s$P0DL
#submission$target <- p0s$Mean
submissionFileName <- paste(outputFolder,prefix,"_dl_submission.csv",sep="")
...
#for(i in 1:4){
  ...
  print(cvAUC::AUC(predictions = predictions , labels = labels)) #0.7474598
  predictions <- predDLVal$p1
  print(cvAUC::AUC(predictions = predictions , labels = labels)) #0.6614806
  predictions <- predRFVal$p1
  print(cvAUC::AUC(predictions = predictions , labels = labels)) #0.7227515
  predictions <- predGBMVal$p1
  print(cvAUC::AUC(predictions = predictions , labels = labels)) #0.7657762
  ...
  p1sVal$Mean <- rowMeans(p1sVal[,c(1,3,4)])
  predictions <- p1sVal$Mean
  # Ensemble test AUC 
  print(cvAUC::AUC(predictions = predictions , labels = labels))#1,4=0.7665595,#1,3,4=
  # 0.7888723
