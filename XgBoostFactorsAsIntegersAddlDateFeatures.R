...
startTime <- Sys.time()
equalSamples <- F
scriptName<-ifelse(equalSamples,"XgBoostEqualSamples","XgBoostAllFactorsAsIntegers3000RoundsDepth50AddnlDateFeatures")
set.seed(13)
os <- Sys.info()[["sysname"]]
...
trainFactorsDF <- data.frame()
testFactorsDF <- data.frame()
ctr <- 1
for(f in usefulFactors){
  combined <- union(unique(levels(training[,f])),unique(as.character(test[,f])))
  if(ctr ==1){
    trainFactorsDF <- as.integer(1:length(combined))[match(training[,f],combined)]
    testFactorsDF <- as.integer(1:length(combined))[match(test[,f],combined)]
  }else{
    trainFactorsDF <- cbind(trainFactorsDF,f=as.integer(1:length(combined))[match(training[,f],combined)])
    testFactorsDF <- cbind(testFactorsDF,f=as.integer(1:length(combined))[match(test[,f],combined)])
  }
  ctr<- ctr+1
}
trainFactorsDF <- as.data.frame(trainFactorsDF)
testFactorsDF <- as.data.frame(testFactorsDF)
colnames(trainFactorsDF) <- usefulFactors
colnames(testFactorsDF) <- usefulFactors
...
train <- training[-which(colnames(training)%in% factorFeatures$Name)]
train <- cbind(splitDateFeatures,trainFactorsDF,train)
...
test1 <- cbind(splitDateFeaturesTest,testFactorsDF,test1)
